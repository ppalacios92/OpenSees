...
### An ADR's early "options considered" section can be SUPERSEDED by its own §12 implementation log -- so grepping one section and concluding "X is missing" produces a confident, wrong finding
- **Bites:** any agent or human surveying what the fork does and does not have by keyword-grepping an ADR. Discovered building [[86_geomechanics_libraries_and_mpm_scoping_report]]: ADR-71 §6 ("Explicit dynamics: the honest note") lists three routes for staggered/explicit u-p -- Zienkiewicz-Paul-Chan 1988, Huang-Zienkiewicz fractional step, Xu et al. 2021 -- and does NOT name the fixed-stress split. Grepping §6 alone yielded the conclusion "fixed-stress is a genuine hole in the option set and the cheapest thing to close", which went into a report's executive summary as its flagship recommendation. It is wrong.
- **Why:** ADR-71 §6 was written at draft time and superseded by ADR-71 **§12** in the SAME FILE (2026-07-13 entry: "The staggered architecture was then tried end-to-end and works: fixed-stress split ... 3 iters vs 11"; and "**P7 OPENED as [[73_ladruno_porous_overlay_adr]]** ... fixed-stress staggered overlay carrying the §6 staggered route"). ADR-73 then SHIPPED it and measured it: "Coupling = fixed-stress staggered split, solid-first" (:112), "≡ monolithic backward Euler: crack probe 0.00 %, Terzaghi 2.8e-8; both sub-solves SPD; factorization ×3.3 cheaper" (:67), with Kim-Tchelepi-Juanes 2011, Mikelic-Wheeler 2013 (the contraction proof) and Turska-Schrefler 1993 all already in its reference list. This fork's ADRs are LIVING documents whose §12 logs routinely overturn their own drafted §§1-9; the drafted text is the plan, the log is the state.
- **Workaround/status (2026-08-23, ADR-86 adversarial review -- caught by a Fable review before the plan was acted on, not by the author):** any claim of the form "the fork lacks X" MUST cite (a) the ADR's §12 implementation log, and (b) the companion ADRs named in its header, before it is written down. A keyword grep over §§1-9 is evidence of what was *planned*, never of what exists. Note the phase-numbering corollary: ADR-71's phase table still lists **P7 as "(research)"**, which is likewise stale -- P7 was opened and carried by ADR-73. The report's §8 carries the generalised rule; §11 carries the full finding.

### Anura3D (LGPL-3) source can NEVER be copied, ported, or hand-translated into this fork -- and the licence file you need to read is `COPYRIGHT`, not `LICENSE`
- **Bites:** anyone harvesting from the open-source geomechanics field for the soil / u-p lanes. Anura3D is the obvious candidate -- it is the only open MPM code shipping production two-phase AND three-phase formulations (`MPMDYN2PhaseSP.FOR`, `MPMDYN3PhaseSP.FOR`, `TwoLayerFormulation.FOR`, `MPMDYNUnsatConsolidation.FOR`) -- and it is on GitHub, which reads as "available".
- **Why:** there is **no `LICENSE` file at this repo root** -- the grant is in `COPYRIGHT`, and it is not BSD. It is a UC Regents grant limited to "educational, research, and non-profit entities for noncommercial purposes only", with everyone else confined to "internal purposes only" and commercial distribution requiring a separate agreement with UC's Office of Technology Licensing. A copyleft licence forbids adding restrictions to the licensed material; that grant adds exactly such a restriction. A hand-translation of a `.FOR` file to C++ is a derivative work of it, so "I rewrote it in my own words" does not help. Clean-room practice exists precisely because reading the source is the contaminating act.
- **Workaround/status (2026-08-23, ADR-86 §2):** Anura3D is **read-only** -- cite it, reproduce its PUBLISHED equations from the papers, do not open its `.FOR` files while writing fork code. Contrast: **OpenGeoSys 6 is BSD-3-Clause** and **CB-Geo `mpm` is MIT** -- both may be reused with attribution; `cb-geo/mpm-benchmarks` is CC-BY-4.0 for inputs and data. **CODE_BRIGHT ships compiled binaries only** (a GiD problemtype), so it is a theory and benchmark reference and there is no source to take. This is an engineer's reading of licence text, not legal advice; a real check is owed before anything Anura3D-shaped becomes code.

### The contact handler guards `ndf == 3` on every node of a pair -- so the whole contact engine is UNAVAILABLE on u-p nodes, and any "contact + LadrunoUP" plan is blocked until that is lifted
- **Bites:** any plan that puts a frictional/separating interface into a poromechanics model -- footing-on-saturated-soil with uplift, a retaining structure with a drained interface, TIMg-UP calibration needing a real interface, or the ADR-86 Track T P5 leg. Also anyone reading the contact ADRs (39/41/47/57/85) and the u-p ADRs (71/73/78) side by side and assuming the two subsystems compose because both are "shipped".
- **Why:** `LadrunoContactFE.cpp` builds its `myID` on the assumption that every node in the pair carries exactly three translational DOFs -- ":100-102" (`SEGMENT` mode: "fills myID = [slave xyz | seg_1 xyz | ... | seg_nps xyz] (each node ndf==3 ...) ... The handler guards ndf==3 on every node of the pair") and ":164" (`MORTAR` mode: "Every node is ndf==3 (the handler guards it) => exact ndof match"). A 3D u-p node carries ndf = 4 (ux, uy, uz, p) -- upstream `SSPbrickUP`/`BrickUP` and the fork's own `LadrunoUP` alike -- so the guard refuses the pair. The guard is correct as written (a wrong `myID` layout would silently assemble contact stiffness into the pressure row, which is far worse than a refusal); it is the DOF-mapping generalisation behind it that does not exist yet.
- **Workaround/status (2026-08-23, ADR-86 §6.4/§8 -- DISCLOSED, not fixed):** for **drained** problems, do not use a u-p element at all -- a single-phase effective-stress model (`LadrunoBrick -formulation bbar` + an effective-stress `nDMaterial`) is ndf = 3 throughout and the contact engine applies immediately. This is the recommended route for TIM/TIM-G calibration, which is drained by construction. For genuinely coupled problems needing an interface, lifting the guard means teaching `LadrunoContactFE` to map only the translational DOFs of a heterogeneous-ndf node set -- tractable (the Domain already runs mixed ndf; see ADR-71 P3's Taylor-Hood vertex-p work) but its own ADR with its own gates, NOT a one-line relaxation.
