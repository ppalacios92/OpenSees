...
#include "LadrunoCohesiveHinge.h"   // Ladruno — discrete cohesive moment-rotation hinge (ADR 32 Tier-2)
...
#include "LogStrain2D.h"           // Ladruno — 2D (plane) Hencky finite-strain adaptor (ADR 25 P5)
...
#include "brick/Twenty_Node_Brick.h"	// Ladruno (ADR-86: broker reconstruction, was missing -- see LEDGER_quirks)
...
#include "tetrahedron/TenNodeTetrahedron.h"	// Ladruno (ADR-86: broker reconstruction, was missing -- see LEDGER_quirks)
...
FEM_ObjectBrokerAllClasses::getNewElement(int classTag)
{
    ...
    case ELE_TAG_Twenty_Node_Brick:	// Ladruno (ADR-86: broker reconstruction, was missing -- see LEDGER_quirks)
    ...
    case ELE_TAG_LadrunoQuad:		// N. Mora-Bowen (Ladruno)
      return new LadrunoQuad();
    ...
    case ELE_TAG_TenNodeTetrahedron:	// Ladruno (ADR-86: broker reconstruction, was missing -- see LEDGER_quirks)
...
FEM_ObjectBrokerAllClasses::getNewUniaxialMaterial(int classTag)
{
	...
	case MAT_TAG_LadrunoCohesiveHinge:            // Ladruno
		return new LadrunoCohesiveHinge();
...
FEM_ObjectBrokerAllClasses::getNewNDMaterial(int classTag)
{
  ...
  case ND_TAG_LogStrain2D:                        // Ladruno — 2D (plane) Hencky finite-strain adaptor (ADR 25 P5)
    return new LogStrain2D();
...
FEM_ObjectBrokerAllClasses::getPtrNewRecorder(int classTag)
{
	...
	case RECORDER_TAGS_LadrunoRecorder:
	  return new LadrunoRecorder();
